...
Strava API → miles-sync → SQLite (data/activities.db)
              ...
              miles-api  (FastAPI on :8000 + vanilla JS UI: Plan · Races · Builds · Compare · Training · Years, plus a Workbook utility;
...
- `miles/mcp_server.py` — 33 MCP tools (table in README): training periods/consistency, race history/PRs/equivalents/splits, fitness estimates, workout laps, `run_sql` escape hatch, and the training-plan draft/commit tools (the only non-read tools; used by `/miles-plan`)
- `miles/plan.py` — training-plan ground truth: one mutable draft per plan (`plan_versions.committed_at IS NULL`), authored incrementally, frozen+validated at `commit_plan`; committed versions are immutable append-only snapshots; contemporaneous `current_version_for_week` keys on committed_at (version 1 floor rule); race auto-complete on sync. Plan tables are athlete-authored — exempt from `derive_all` rebuilds. See `adr/0001-training-plans-as-versioned-ground-truth.md`
...
- `miles/plan_api.py` — `/api/plan`, `/api/plan-adherence`, `/api/plan-progression`, `/api/plan-retrospective`, `/api/plan-readiness` (goal-vs-fitness trend, current-block-vs-reference-build comparison aligned at the same weeks-out cutoff, session execution), versions/diff endpoints backing plan.html
...
- `miles/static/` — vanilla JS, no framework. `plan.html` is the landing page (`/` redirects here) — the training plan vs reality: a Today tab (session card, week at a glance, week-so-far vs target pro-rated by planned mileage, same-weekday-cutoff comparison to last week — all cut off at last_sync_at on both sides) and an Overall tab (Readiness section for active plans: goal-vs-fitness, current-block-vs-reference-build with athlete-flippable reference chips, session execution; planned-vs-actual weekly chart with adherence bands + flag shading, slot-color-coded weekly calendar with clickable day-detail popovers and visible week notes, easy-HR/workout-pace progression charts, version history with diffs); a multi-session date (plan_days seq > 1 — a double run, or a run plus a strength day) renders every session everywhere (calendar chip, popover, Today card/week-strip), never just the last row; below 640px the calendar collapses from its grid to stacked full-width week cards, no horizontal scrolling; renders a race retrospective for a completed plan, an empty state when no plan exists. `races.html`: Overview tab (PB stat strip, race-paces scatter with standing-PR labels, all-races archive) plus a per-distance tab (5K/10K/Half/Marathon/50K/Other), each with a stat strip (PR · Latest · Races · the two nearest sub-X barriers the PR has cracked), 3-mode chart, and a distance-aware sortable table (LR columns on long distances only, distance-relevant pace claims). `builds.html` is the canonical builds index (race result Time column, casual efforts muted; `#build-{date}` deep-links flash the row). `build.html#{race-date}` is the drill-down (stat strip, weekly calendar, lap panel, workout-group comparisons; falls back to a fixed distance-bucket window for races without a detected build). `compare.html` is a build-vs-build workbench (max 4, per distance-bucket). `training.html` is fitness + current load: stat strip (Fitness follows the chart's distance toggle), per-distance Fitness chart with hover/click-to-pin evidence (markers encode basis: solid = race-confirmed, ring = workout-anchored, hollow = training floor), an Efficiency HR-vs-pace section (see below), and the full weekly-mileage chart with period/build bands. `years.html` is year-over-year volume (Cumulative default; multi-select year chips — tiles and the races table follow the most recently selected year). `workbook.html` (header ⌗ icon, not a nav link) collects one-off Claude-authored analysis pages uploaded via `/api/workbooks` (title + upload date). The athlete's reference-build choice persists in localStorage as `miles.refBuild.{bucket}` (race date; PR-build fallback) and is shared by plan/builds/build pages. `design-lab.html` + `static/lab/*.js` are a kept-around design playground, not a shipped page. `theme.css` (tokens + shared table/tab/nav classes) and `nav.js` (header nav: Plan · Races · Builds · Compare · Training · Years + Workbook icon + Sync; `chartTheme()` for ECharts) are shared by every page; `charts.js` holds shared `fmt`/color/`staggerEndLabels`/`sparklineSVG`/`makeSortable` helpers
...
### HR-vs-pace heatmap (training.html "Efficiency" section)
